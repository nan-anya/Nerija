...
public class Alarm extends Thread implements Serializable
{
    ...
    public void StartAlarm(final Context app, final Intent intent, final AlarmBaseData alarmBaseData)
    {
        ...
        final Handler handler = new Handler()
        {
            ...
            {
                check(app,vib,intent,msgMng,alarmBaseData,lc);  // 시간 차이 구하는 메소드 실행
            }
        };
    ...
    // 시간차이 구하는 메소드
    public void check(final Context app, Vibrator vib, Intent intent, MessageManager msgMng, AlarmBaseData alarmBaseData, LocationChecker lc)
    {
        Toast.makeText(app,""+alarmBaseData.arrivalPlaceName + " : " + alarmBaseData.getDate().toString(),Toast.LENGTH_SHORT).show();

        //long diff = new Date().getTime() - alarmBaseData.selectedDate.getTime();    // 현재시간 - 받아온 시간 => 실제로는 받아온 시간 - 현재시간 할 것
